...
const getAllArticlesPreviews = async (): Promise<ArticlePreview[]> => {
  ...
  return cacheGet("articles:previews:all", async () => {
    const articles = await Article.find<ArticleType>({}).select(
      `_id slug featuredImage translations.be.title category translations.be.tags translations.be.excerpt publishedAt`,
    );
    return articles.map((article) => {
      ...
      return {
        ...
        category,
        featuredImage,
        title: article.translations[Languages.Belarusian].title,
      };
    });
  });
};

const getArticlePreviewsByFilter = async (
  language: string = Languages.Belarusian,
  categoryId?: string,
...
const getFullArticlesByFilter = async (
  language: string = Languages.Belarusian,
  categoryId?: string,
): Promise<ArticleType[]> => {
  await setupMongooseConnection();
  ...
  const filter = categoryId ? { category: categoryId } : {};
  return cacheGet(`articles:full:${language}:${categoryId || "all"}`, async () => {
    return await Article.find<ArticleType>(filter);
  });
};
